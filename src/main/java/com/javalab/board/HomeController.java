...
public class HomeController {
	 ...
	 * [@ResponseBody]
	 * - 호출한 곳으로 응답할 때 "메시지 바디"에 담아서 전달해주면 받는
	 ...
	 * 	 Java 객체를 JSON 데이터로 변환(직렬화)하는 기능을 제공합니다.
	 * - Map<String, Object> : Map 타입으로 Key-value 형태를
	 * 	 JSON Type 문자열로 반환
	 */
	@RequestMapping(value="/action3.do/name1/{name2}/grade1/{grade2}", method= {RequestMethod.GET, RequestMethod.POST})
	public @ResponseBody Map<String, Object> action3(@PathVariable("name2") String name, @PathVariable("grade2") int grade,
															Model model) {
