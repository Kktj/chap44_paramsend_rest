 ...
 * - @Responsebody가 없어도 자동으로 반환 데이터에 붙어서 요청 화면의 바디에 써줌
 * - @RestController = @Controller + @Responsebody
 */
@RestController // @Controller + @Responsebody 결합된 컨트롤러
@Slf4j
public class RestControllerDemo {
	/*
	 * @RestController의 모든 메소드는 @ResponsBody가 자동으로 붙음(보이지 않지만)
	...
	public MemberVo action4(@PathVariable("name") String name, @PathVariable("grade") int grade, Model model) {
		...
		// 화면에서 전달받은 값으로 새로운 객체를 생성해서 그대로
		// 화면으로 다시 전송
		MemberVo member = new MemberVo();
		member.setName(name);
		member.setGrade(grade);
		
		return member; // JSON Type 문자열로 변환(@ResponseBody + Jackson-databind)
		
	}
	/*
	 * @RestController의 모든 메소드는 @ResponseBody가 자동으로 붙음
	 * - @ResponseBody : 요청한 페이지의 바디에 출력
	 * - @RequestBody MemberVo : JSP에서 ajax 형태로 요청한 값들을 커맨드 객체 자동 바인딩
	 */
